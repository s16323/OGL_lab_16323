...
GLuint BaseProgram::LinkProgramOrDie(GLuint vertex_shader, GLuint fragment_shader){
    ...
    if ( !linked ) {
        ...
        char* log_msg = new char[static_cast<unsigned>(log_size)]; //++
...
GLuint BaseProgram::LoadAndCompileShaderOrDie(const char * source_file, GLenum type){
    ...
    if (file.is_open()) {
        ...
        glShaderSource(shader, 1, reinterpret_cast<GLchar**>(&shader_code), nullptr); //++
    ...
    if (!compiled) {
        ...
        //char* log_msg = new char[log_size]; /--
        char* log_msg = new char[static_cast<unsigned int>(log_size)]; //++
